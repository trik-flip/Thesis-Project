...
def setup_module(module):
    print("")
    print("%s" % (setup_module.__name__,))


# Can also be teardown, teardown_module, or tearDownModule
def teardown_module(module):
    print("%s" % (teardown_module.__name__,))
...
class test_class_1:
    ...
    def setup_class(cls):
        print(
            "%s called before any method in this class is executed"
            % (test_class_1.setup_class.__name__,)
        )

    @classmethod
    def teardown_class(cls):
        print(
            "%s called after methods in this class is executed"
            % (test_class_1.teardown_class.__name__,)
        )
